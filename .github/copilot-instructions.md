# Copilot code review — real_state_site_template
...
- **Themes are skins.** `src/themes/*` receive already-resolved values (listings, `SiteConfig`, locale, copy) and paint. Reuse `ListingInquiryForm`, `ListingWhatsAppButton`, `publicListingCardModel`, `listingPublicSpecsLocalized`, `PublicCatalogSearch`. Do not copy inquiry POST, `wa.me` parsing, or specs mapping into a theme.
- **No theme branching in `src/app/`.** Routes fetch + delegate to `THEME_REGISTRY`. No `if (theme.name === …)` in `page.tsx`.
...
- **Generic template.** No client names, cities, or exclusive copy in shared code. Branding comes from `SiteConfig`.
