...
export function UltraPagination({
  page,
  totalPages,
  total,
  oferta,
  ...
  return (
    <nav className="ultra-pagination" aria-label={copy.paginationAria}>
      <div className="flex items-center justify-between gap-3 md:hidden">
        {canPrev ? (
          <Link
            href={hrefFor(page - 1)}
            className="ultra-page-btn"
            aria-label={copy.paginationPrev}
          >
            {copy.paginationPrev}
          </Link>
        ) : (
          <span className="ultra-page-btn is-disabled" aria-disabled="true">
            {copy.paginationPrev}
          </span>
        )}
        <p className="ultra-pagination__status min-w-0 flex-1 text-center">
      ...
      <div className="hidden flex-wrap items-center justify-center gap-2 md:flex">
        {canPrev ? (
          <Link href={hrefFor(page - 1)} className="ultra-page-btn">
            {copy.paginationPrev}
          </Link>
        ) : (
          <span className="ultra-page-btn is-disabled" aria-disabled="true">
        ...
        {items.map((item, index) =>
          item === "ellipsis" ? (
            <span
              key={`e-${index}`}
              className="ultra-pagination__ellipsis"
              aria-hidden
            >
              …
            </span>
            ...
            <Link
              ...
              href={hrefFor(item)}
              className={
                item === page ? "ultra-page-btn is-active" : "ultra-page-btn"
              }
