...
export function listingMatchesLocation(
  ...
  const source = listing.city?.trim()
    ? listing.city
    : listing.location_label || "";
  return cityKey(source) === needle;
}

export function elegantLocationsGridClass(count: number): string {
  if (count <= 1) return "elegant-locations__grid elegant-locations__grid--single";
  return "elegant-locations__grid";
}
